...
if ( ! class_exists( 'WPSEO_Local_Core' ) ) {
	...
	class WPSEO_Local_Core {
		...
		public function add_to_index( $str ) {
			...
			$str .= '<sitemap>' . "\n";
			$str .= '<loc>' . WPSEO_Sitemaps_Router::get_base_url( 'geo-sitemap.xml' ) . '</loc>' . "\n";
		...
		public function build_local_sitemap() {
			...
			$output = '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:geo="http://www.google.com/geo/schemas/sitemap/1.0">
				<url>
					<loc>' . WPSEO_Sitemaps_Router::get_base_url( 'locations.kml' ) . '</loc>
		...
		public function save_permalinks_on_option_save( $old_value, $new_value ) {
			...
			if ( isset( $old_value['locations_slug'] ) && $old_value['locations_slug'] == $new_value['locations_slug'] ) {
				return;
			}
		...
		public function get_location_data( $post_id = null ) {
			...
			if ( wpseo_has_multiple_locations() ) {
				...
				$business = array(
					...
					'business_address'     => isset( $options['location_address'] ) ? $options['location_address_2'] : '',
					'business_address_2'   => isset( $options['location_address_2'] ) ? $options['location_address'] : '',
					...
					'business_url'         => WPSEO_Sitemaps_Router::get_base_url( '' ),
					'coords'               => array(
			...
			$locations['kml_website']   = WPSEO_Sitemaps_Router::get_base_url( '' );
