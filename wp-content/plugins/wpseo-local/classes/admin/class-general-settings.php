...
if ( ! class_exists( 'WPSEO_Local_Admin_General_Settings' ) ) {
	...
	class WPSEO_Local_Admin_General_Settings {
		...
		public function single_location_settings() {
			...
			WPSEO_Local_Admin_Wrappers::textinput( 'location_url', __( 'URL', 'yoast-local-seo' ), '', array( 'placeholder' => WPSEO_Sitemaps_Router::get_base_url( '' ) ) );
			...
			WPSEO_Local_Admin_Wrappers::textinput( 'location_price_range', __( 'Price range', 'yoast-local-seo' ) );
		...
		public function business_image() {
			...
			echo '<img src="' . ( isset( $this->options['business_image'] ) ? wp_get_attachment_url( $this->options['business_image'] ) : '' ) . '" id="business_image_image_container" class="wpseo-local-hide-button' . ( ( false == $business_image ) ? ' hidden' : '' ) . '">';
		...
		public function opening_hours() {
			...
			foreach ( $this->wpseo_local_core->days as $key => $day ) {
				...
				WPSEO_Local_Admin_Page::section_before( 'opening-hours-' . $key, null, 'clear opening-hours' );
				...
				WPSEO_Local_Admin_Page::section_before( 'opening-hours-second-' . $key, null, 'clear opening-hours-second ' . ( ( empty( $this->options['multiple_opening_hours'] ) || $this->options['multiple_opening_hours'] != 'on' ) ? 'hidden' : '' ) . '' );
